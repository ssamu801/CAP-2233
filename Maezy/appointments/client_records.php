							...
							<tbody>
				                <?php
									...
									$counselor_email = $_SESSION['login_email'];
 					                ...
 					                $users = $conn->query("SELECT u.id, u.name, u.email FROM users u JOIN events e ON u.id=e.student_id WHERE e.counselor_email ='$counselor_email' GROUP BY u.id ORDER BY u.name ASC");
 					                $i = 1;

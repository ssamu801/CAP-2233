...
if(isset($_GET['id'])){
    ...
    if($_GET['type'] == 6){ // confirmation of appointment submission
        $sql = "SELECT e.mode, e.date, e.time_to, e.time_from FROM notifications n JOIN events e ON n.event_id=e.id WHERE n.id=$id LIMIT 1;";
        $result = $conn->query($sql);
        if ($result && $result->num_rows > 0) {
            $row = $result->fetch_assoc();
            echo "Your request is now waiting for approval.<br><br>"; 
